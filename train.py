...
def train(opt, show_number = 2, amp=False, logs=True):
    """ dataset preparation """
    ...
    train_dataset = Batch_Balanced_Dataset(opt, logs=logs)

    log = open(f'./saved_models/{opt.experiment_name}/log_dataset.txt', 'a', encoding="utf8")
    AlignCollate_valid = AlignCollate(imgH=opt.imgH, imgW=opt.imgW, keep_ratio_with_pad=opt.PAD, contrast_adjust=opt.contrast_adjust)
    valid_dataset, valid_dataset_log = hierarchical_dataset(root=opt.valid_data, opt=opt, logs=logs)
    ...
    t1= time.time()
    run = True  
    every = round(opt.num_iter/10)
    factor = round(10/opt.num_iter)
    ...
    while run:
        ...
        if (i % opt.valInterval == 0) and (i!=0):
            ...
            with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a', encoding="utf8") as log:
                model.eval()
                with torch.no_grad():
                    valid_loss, current_accuracy, current_f1, current_norm_ED, preds, confidence_score, labels,\
                ...
                # keep best fi score model (on valid dataset)
                if current_f1 > best_f1:
                    best_f1 = current_f1
                    torch.save(model.state_dict(), f'./saved_models/{opt.experiment_name}/best_f1.pth')
