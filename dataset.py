    ...
    def __init__(self, opt, logs):
        ...
        for selected_d, batch_ratio_d in zip(opt.select_data, opt.batch_ratio):
            ...
            if logs:
                print(dashed_line)
            log.write(dashed_line + '\n')
            _dataset, _dataset_log = hierarchical_dataset(root=opt.train_data, opt=opt, select_data=[selected_d], logs=logs)
...
def hierarchical_dataset(root, opt, select_data='/', logs=True):
    ...
    for dirpath, dirnames, filenames in os.walk(root+'/'):
        if not dirnames:
            ...
            if select_flag:
                dataset = OCRDataset(dirpath, opt, logs=logs)
...
class OCRDataset(Dataset):
    ...
    def __init__(self, root, opt, logs):
